...
[[nodiscard]] sus_pure_const consteval sus_always_inline size_t size_of() noexcept {
  return sizeof(T);
}
...
template <class T>
  requires(!std::is_reference_v<T>)
[[nodiscard]] sus_pure_const consteval sus_always_inline size_t data_size_of() noexcept {
