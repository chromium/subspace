...
class Vec final {
 ...
 public:
             ...
             ::sus::mem::IsMoveRef<decltype(ii)>)
  {
    ...
    usize lower = it.size_hint().lower;
    if constexpr (sus::iter::ExactSizeIterator<decltype(it)>) {
      reserve_internal(lower);
      if constexpr (sus::iter::TrustedSizeIterator<decltype(it)>) {
        usize i = len_;
        for (auto&& t : it) {
        ...
        usize i = len_;
        for (auto&& t : ::sus::move(it).take(lower)) {
          std::construct_at(data_ + i, t);
          i += 1u;
        }
        len_ += i - self_len;
      }
    } else if (sus::Option<const T&> first = it.next(); first.is_some()) {
      const usize r = sus::ops::max(lower, 1_usize);
      reserve_internal(r);
      usize i = len_;
      std::construct_at(data_ + i, sus::move(first).unwrap());
      i += 1u;
      for (const auto& t : it.by_ref().take(r - 1u)) {
        std::construct_at(data_ + i, t);
        i += 1u;
      }
      len_ += i - self_len;
    ...
    requires(::sus::mem::IsMoveRef<decltype(ii)>)
  {
    ...
    if constexpr (sus::iter::ExactSizeIterator<decltype(it)>) {
      ...
      if constexpr (sus::iter::TrustedSizeIterator<decltype(it)>) {
        usize i = len_;
        for (auto&& t : it) {
          std::construct_at(data_ + i, ::sus::move(t));
          i += 1u;
        }
        len_ += i - self_len;
      } else {
    ...
    } else if (sus::Option<T> first = it.next(); first.is_some()) {
      const usize r = sus::ops::max(lower, 1_usize);
      reserve_internal(r);
      usize i = len_;
      std::construct_at(data_ + i, sus::move(first).unwrap());
      i += 1u;
      for (auto&& t : it.by_ref().take(r - 1u)) {
        std::construct_at(data_ + i, ::sus::move(t));
        i += 1u;
      }
      len_ += i - self_len;
...
#include "__private/slice_mut_methods.inc"
 ...
 private:
  ...
  /// Requires that:
  /// * Vec is in a valid state to mutate
  constexpr void reserve_internal(usize additional) noexcept {
    if (len_ + additional > capacity_) {
      if (!is_alloced())
        alloc_internal_check_cap(additional);
      else
        return grow_to_internal_check_cap(apply_growth_function(additional));
    }
  }

  /// Requires that:
  /// * Vec is in a valid state to mutate
  constexpr void reserve_exact_internal(usize additional) noexcept {
    const usize cap = len_ + additional;
    if (cap > capacity_) {
      if (!is_alloced())
        alloc_internal_check_cap(cap);
      else
        grow_to_internal_check_cap(len_ + additional);
    }
  }
  ...
  /// * Vec is in a valid state to mutate
  constexpr void reserve_allocated_internal(usize additional) noexcept {
  ...
  /// * Vec is in a valid state to mutate
  constexpr void alloc_internal_check_cap(usize cap) noexcept;

  /// Requires that:
  /// * `cap` > `capacity()`
  /// * Vec is already allocated
  /// * Vec is in a valid state to mutate
  constexpr void grow_to_internal_check_cap(usize additional) noexcept;
...
template <class T>
constexpr void Vec<T>::alloc_internal_check_cap(usize cap) noexcept {
  sus_debug_check(!is_alloced());
...
template <class T>
constexpr void Vec<T>::grow_to_internal_check_cap(usize cap) noexcept {
  sus_debug_check(is_alloced());
